...
\begin{abstract}
What does identifying a form as an English interjection let speakers,
hearers, and analysts infer? This paper argues that interjections form a
stable causal-pragmatic projectible cluster. Their strongest
inferences concern prosodic packaging, syntactic supplementarity,
use-conditional stance, sequential position, and interactional uptake.
They're projectible not because they share an essence, and not merely
because they resemble a prototype, but because different recruitment
pathways converge on a causal-pragmatic profile while leaving residues
that structure boundary behaviour.

The analysis is path-dependent: a form's route into the interjectional
profile continues to shape its later behaviour. Invocative, verbal,
onomatopoeic, and regional/indexical routes leave different residues,
which support diagnostic historical inference. Supplementary GloWbE and
COHA probes illustrate boundary behaviour, exit residue, and attrition,
but they do not establish corrective control. Interjections therefore
support the Boydian lesson that fuzzy categories can be inductively
useful while showing why homeostasis should not be used as a synonym
for durable clustering.
\end{abstract}

\noindent Keywords: interjections; historical pragmatics;
grammaticalization; pragmaticalization; pragmatic markers; causal
networks; projectibility; stable clusters; path dependence
...
words of other categories may be used.
...
lexical categories in their large reference grammar (\mention{ah},
...
This paper shifts the question from
\enquote{are interjections a category?} to \enquote{what does
categorizing something as an interjection let you predict?} I use
\term{projectibility} for this inferential payoff
\citep{Goodman1955}: observing some properties of a new case licenses
predictions about others. A category with no projectibility is a filing
label. One that supports reliable inferences across novel instances is
doing theoretical work. The same pattern matters inside the language:
learners and hearers can use partial cues to anticipate how a form will
behave. For interjections, prosodic isolation or non-inflection should
license expectations about syntactic independence, non-referentiality,
and interactional force.

The object of analysis is therefore the category position, not just the
membership list. Individual items such as \mention{wow}, \mention{damn},
\mention{bruh}, and \mention{fie} matter because they show what the
category lets speakers and hearers infer: likely prosody, syntactic
independence, limits on argumenthood, stance or response function, and
possible pathways into or out of the profile.

By \term{path dependence}, I mean that a form's route into the
interjectional profile continues to shape its later boundary behaviour.
Convergence on the profile does not erase source history: invocative,
verbal, onomatopoeic, and regional/indexical routes leave different
residues, and those residues support diagnostic historical inference.

Throughout, I use \term{property cluster} descriptively for the
empirical pattern of co-occurring features. The public claim is a
calibrated one: English interjections form a \term{stable projectible
cluster} and a \term{causal-pragmatic kind}. Whether the cluster is
\term{homeostatic} in the stricter sense remains a further maintenance
hypothesis.

That inferential question motivates a causal-network analysis.
...
the claim is that prosodic isolation, syntactic independence,
non-referentiality, non-inflection, and interactional force cohere
because several causal pathways converge on the same pragmatic node.
The resulting profile is thin but projectible.

The evidential order matters. The analysis first identifies a
\term{projection target}: what categorization as an interjection lets
speakers, hearers, and analysts infer. It then asks whether a
\term{stable profile} supports that target, what \term{network order}
makes the profile non-accidental, what \term{stabilizers} may maintain
it, and whether any stabilizer amounts to \term{corrective control}.
Only the final step would license strict homeostatic language.

For historical pragmatics, this reframes category status as a question
about communicative use over time. The relevant evidence includes forms
such as \mention{gee}, \mention{damn}, or \mention{bruh} changing
category labels and historically contentful or addressive expressions
coming to manage stance, sequence, and interactional alignment in
recurrent contexts \citep[cf.][]{juckertaavitsainen2013}. Diachronic
change is the route by which the pragmatic category becomes visible.

The network classifies tokens in context, not lexemes. A lexeme may
supply several historically related forms, but projectibility attaches
to the token-level profile. \mention{Damn} as a standalone interjection,
\mention{damn} as an attributive modifier in \mention{the damn car}, and
\mention{damn} as a verb in \mention{I damn you} have different cluster
profiles. They may therefore occupy different nodes in the causal
structure, or the same node in different degrees. Lexemes are
convenient labels for these token-level profiles.

The label \enquote{interjection} therefore covers the overlap among
...
framework and explains why it handles the interjection data better than
classical or prototype-based alternatives. The rest of the paper
identifies the network order that structures the cluster, extending
\textcite{gehweiler2008}'s single-case analysis to a general
theoretical claim, and argues that interjections
...
and large-corpus work, but they're not direct acoustic evidence.
...
inflection. The construction is better treated as a residue of verb syntax than as
full verbal behaviour, a point that matters for the recruitment pathways
discussed in Section~\ref{sec:causal-structure}. (The internal structure of lexicalized pairs like
...
While nouns and verbs typically pick out entities or events in the
world, interjections express internal states of their users
...
\enquote{bleached of [its] original meaning}, and it no longer refers to
...
felicity conditions. The same form can therefore sit close to one node
for one inquiry and farther away for another, depending on which causal
relations and predictions the inquiry tracks.

The projective claim should therefore be level-specific. A novel
...
loses prosodic isolation and syntactic non-integration, since it's embedded
inside an NP \citep[cf.][]{potts2007}. When the links that couple
the properties are neutralized (here, prosodic-syntactic independence),
the properties erode together. A list of
co-occurring properties can't explain this pattern. A cluster whose
co-occurrence is linked by identifiable processes can.

Table~\ref{tab:diagnostic-matrix} makes the cluster inspectable.
Each row represents tokens in a constructional environment, not a lexeme
as a whole (see Section~\ref{sec:intro}), and the columns show its
...
reveals where the nodes converge (standalone \mention{damn!}, \mention{oh}) and
where they decouple (attributive \mention{the damn car} is
\term{interjection\textsubscript{sem}} but not
\term{interjection\textsubscript{syn}}; \mention{mm-hm} is
\term{interjection\textsubscript{int}} but debatably
\term{interjection\textsubscript{syn}}). Boundary items (\mention{bye},
\mention{um}) share some but not all cluster properties, the pattern
that Section~\ref{sec:boundaries} explains as causal-link overlap.
...
\section[Causal-network kinds]{Causal-network kinds and projectibility}\label{sec:causal-framework}
...
conditions, graded boundaries, and hedged universals. Classical category
theory has trouble with that profile, which makes causal-network
analysis a better fit.

\subsection{The framework}

\textcite{khalidi2013,khalidi2018nodes} argues that natural kinds are
nodes in causal structures. A kind can be unified by causal relations
rather than by an essence. Those relations may be reciprocal feedbacks,
one-directional pathways, or converging historical routes. What matters
is that the category marks a place in a causal network: a position where
properties, processes, and interventions are connected tightly enough to
support induction. The clustering matters because it lets us predict
properties of new instances from observed properties of old ones.
...
I use the framework as a diagnostic ladder. First, what's the
...
The comparison with \term{homeostatic-property-cluster theory} is
therefore lineage rather than headline verdict. \textcite{boyd1999}
showed that fuzzy categories can be scientifically useful and
projectible when their properties are connected non-accidentally.
Interjections support that Boydian lesson: they lack necessary and
sufficient conditions, but show recurrent co-occurrence among prosodic,
syntactic, semantic, and pragmatic diagnostics.

That recurrence is not enough to establish homeostasis. Prosodic
isolation and syntactic non-integration do form a local stabilizing
relation. A form produced as its own intonation unit is less available
for complementation or modification. A form outside clause structure is
more likely to receive independent prosodic packaging. Repeated use in
that position can make standalone interpretation increasingly expectable
for speaker and hearer.

But this relation shows stabilization, not corrective control. Other
links are directional rather than mutually stabilizing: loss of
referential content makes argument structure less motivated; syntactic
deactivation pushes a form toward standalone use; repeated conversational
deployment entrenches interactional function. These relations may
produce stable co-occurrence, though they do not show a system or
practice registering perturbations and restoring a higher-level
category profile \citep[cf.][]{weinberger2026HomeostasisCausalControl}.

A third part of the evidence is historical convergence: semantic
bleaching, syntactic deactivation, and onomatopoeic coinage can all
recruit forms into a similar pragmatic profile. The resulting category
is organized by several causal-pragmatic relations with different
temporal and explanatory profiles.

The point can be put concretely. If a form occupies its own intonation
unit, it becomes less available as a complement or modifier. If bleaching
removes referential content, argument structure becomes less motivated.
If a form is repeatedly used for stance, response, repair, or sequence
management, interactional conventionalization reinforces standalone use.
These processes can stabilize one another once the form is already
functioning interjectionally, though they need not originate in the same
link. The diagnostics travel together because they're linked within a
causal-pragmatic structure.

Homeostatic-property-cluster theory keeps the projectibility lesson
visible, but the causal-network analysis captures the mixed structure
more directly: feedback, directional dependency, diachronic recruitment,
and boundary failure without assuming that every durable cluster is
homeostatic.
...
fuzzy boundaries via overlapping similarity spaces. These are core
predictions of prototype theory, and all are confirmed by the
interjection data. Prototype theory
also correctly predicts that boundary disputes (interjections vs.\
fillers, vs.\ routine formulae) should recur rather than
resolvable by definition. These are genuine explanatory achievements,
and the causal-network account doesn't replace them.
...
organized around better and worse exemplars. It does not explain why
the properties cluster in this particular pattern, or what should happen
when one of the links among them is disrupted.

That difference matters because the causal-network account generates
...
links are real, messy boundary cases should not be messy in arbitrary
...
A stronger alternative is a \term{feature specification}, for example
...
bundles, and constructional representations do not settle by themselves:

\begin{enumerate}
\item Co-movement. Prosodic isolation, syntactic supplementarity,
  non-referentiality, and pragmatic force do not merely recur. The
...
\item Asymmetry. Some properties are upstream or more diagnostic than
  ...
  feature bundle cannot represent that ordering.
...
\item Boundary behaviour. Different recruitment pathways should leave
  ...
  residual properties but does not explain why they align with
...
\item Failure. When a prediction fails, the pattern of failure should
  ...
  loses all three nodes together under community-wide attrition, that's
  a boundary condition on ordered erosion, not evidence of corrective
  control.
\end{enumerate}
...
point of view of} another} \citep[150]{boyd1999}. The present paper
treats that formulation as a useful gloss on the causal-network account.
...
with properties such as subject--auxiliary inversion, \mention{wh}
fronting, and polar-question syntax. Pragmatists track \term{question}:
...
\item \term{interjection\textsubscript{sem}}, the semanticist's
  ...
  Projects meaning type. Different extension: \mention{damn} in
...
pragmaticalization matter here because they alter a form's communicative
job as well as its position in a syntactic tree
...
\ea\label{ex:bleaching}
  ...
  \z
\z
The stages layer cluster properties unevenly. Prosodic isolation is
already present at stage~1, because a vocative like
...
The trajectory is constrained, not a random walk. Each stage makes the
...
independence (stage~4). Gehweiler's corpus data documents the
...
pressures. The causal-network claim therefore doesn't require Gehweiler's sequence
...
the same cluster profile supports the network-order claim:
the same node can be reached by different routes.
...
ability. They function as standalone interjections
with no residual argument structure.

\mention{Damn} represents partial syntactic
...
meaning, with hearers interpreting the form as a conventional expression, not
...
\subsection{Prosodic isolation and syntactic non-integration}
...
inside the NP and receives the prosody of the phrase.

The prediction is therefore concrete. If a novel form is reliably
...
\subsection{The non-referentiality ratchet}
...
therefore keeps the form near the interjection node by blocking return
to the source category, while prosodic-syntactic coupling links the
central structural properties.

\subsection{Interactional routine entrenchment}
...
The interactional routine therefore favours short, standalone,
...
\subsection{Regional/indexical conditioning}
...
does not by itself establish social indexing in the stronger
...
Regional/indexical conditioning is therefore a maintenance hypothesis.
...
the causal structure of the kind. This comparison remains unexecuted.

The interjection cluster therefore has two structural ordering links
...
source. \\
\addlinespace
Onomatopoeic or embodied coinage & Iconic vocalization &
...
iconic coinage. \\
\addlinespace
Interactional or regional/indexical recruitment & Sequential slot or community marker &
...
This turns apparent exceptions into evidence. \mention{Damn these
...
It does not predict that different boundary failures should line up with
...
\footnote{The analysis plan was committed before extraction and coding
  ...
  data, and code are archived at [URL removed for blind review].\else
...
\footnote{The analysis plan was committed before data extraction
  ...
  pre-registration, data, and code are archived at [URL removed for
...
Regional variation illustrates the same point from a geographic angle.
...
lexemes also have verbal or nominal uses. The category position remains
...
the space of possible resources}. The causal-network account
describes that space as a property space structured by
the recruitment pathways and synchronic links above.

Having shown how the cluster is recruited, ordered, and replenished,
we can now ask what that structure lets speakers, hearers, and analysts
infer from partial evidence.
...
\term{projectibility} means forward inference from observed category
membership to further behaviour.

Projectibility also helps make a language learnable and usable:
speakers and hearers use partial cues to anticipate how a form will
behave before they've observed its full distribution. Linguists then
exploit the same structure for field-specific purposes.

The answer is the paper's central claim:
interjections are a \term{causal-pragmatic kind} whose richest
...
The clearest running case is \mention{damn}. Standalone
\mention{damn!}, complement-taking \mention{damn these mosquitoes!}, and
attributive \mention{the damn car} share a lexeme but occupy different
positions in the network. That contrast makes the methodological point
concrete: the network classifies tokens in context, not lexemes.
...
has interjection properties. That's trivially true of any category. Causal-network
...
use for the first time observes that it's prosodically isolated and
carries stance meaning. From those two observations alone, the three
field-relative nodes license a battery of further expectations:
...
\ea\label{ex:bruh}
  \ea \term{interjection\textsubscript{syn}}: the interjectional use
    ...
    (*\mention{the bruh}, *\mention{Bruh surprised me}). This does not
    rule out separate nominal or verbal uses of the same lexeme.
...
\item Near-entailments (a): the
  \term{interjection\textsubscript{syn}} predictions follow from the observed
  properties themselves (non-inflection, non-referentiality), not from
  causal structure. Any framework recognizing these properties generates
  the same predictions. These are real but not specific to the
  causal-network account.
...
different causal links and different inferential payoffs. This payoff
comes from treating kinds as nodes in causal structures: a field tracks
the node whose causal relations support its explanations and predictions.
The three interjection nodes introduced in
Section~\ref{sec:fieldrelative-nodes} therefore pay off as different
packages of inference rather than as rival definitions.
...
apart because other links are field-specific. Attributive
\mention{damn} is \term{interjection\textsubscript{sem}} but not
\term{interjection\textsubscript{syn}}; \mention{mm-hm} is
\term{interjection\textsubscript{int}} but debatably
\term{interjection\textsubscript{syn}} \citep{oconnell2005}. The same word can
therefore matter differently for syntax, semantics, and interactional
pragmatics. That's exactly what the three-node analysis is meant to
capture.

The architecture generates a cross-node prediction: because some
links contribute to all three nodes simultaneously, membership in
one should predict membership in the others. A novel form categorized as
\term{interjection\textsubscript{syn}} (prosodically isolated, non-inflecting,
syntactically non-integrated) should also tend to be
\term{interjection\textsubscript{sem}} (use-conditional, speaker-oriented) and
\term{interjection\textsubscript{int}} (turn-initial, available for
backchanneling).

That's the default, and it holds for most interjections
(\mention{wow}, \mention{ouch}, \mention{hey}). The interesting cases
are where the prediction fails: attributive \mention{damn} is
\term{interjection\textsubscript{sem}} but not
\term{interjection\textsubscript{syn}}; \mention{mm-hm} is
\term{interjection\textsubscript{int}} but debatably
\term{interjection\textsubscript{syn}}. Each failure identifies a point where
the links decouple, and the pattern of decoupling is itself predicted
by which links are field-specific.

The decomposition also has empirical force. It generates predictions
that the individual frameworks can't produce in isolation. The three
constructions of \mention{damn} illustrate this.
...
For attributive \mention{the damn car}, they
...
semantics alone predicts this gradient. The three-node analysis
does, because it tracks which links are active in each
construction.

\subsection{Category vs.\ function}

A question from the \textit{CGEL} tradition sharpens the projectibility point:
what does the \term{interjection\textsubscript{syn}} category label add
beyond the \term{supplement} function label? Supplements of any category (NPs, AdjPs,
...
label tells you about inference, licensing a package of predictions
...
will face the same question, and the causal-network answer is the same. The
...
The three-node analysis reveals something about causal-network kinds.
...
\term{interjection\textsubscript{int}}, the interactional-pragmatic cluster.
...
management. The forms at the intersection of all three nodes are what I call a
\term{causal-pragmatic kind}: their strongest inferences are about how
they'll be used rather than what they'll denote.
...
The domain follows from which links happen to converge. Noun projectibility is morphosyntactic
because the underlying processes (agreement, structural analogy,
...
toxicity) but does not normally project grammatically in the way animacy
...
why keep \term{interjection\textsubscript{syn}} in the grammar at all?
The answer is that the thin \term{interjection\textsubscript{syn}} node is
projectible and has its own causal links. Neutralize prosodic isolation and the form is
...
The payoff is historical-pragmatic: boundary failures diagnose
recruitment routes.
...
string search, and the coefficient of variation (CV = SD/mean) was
...
effects for item and country) supports
...
The CV table is the primary evidence; the model shows that the pattern
survives a crossed random-effects structure. Full model specification
...
functions as an adverb rather than a standalone interjection
...
The probes have different weights. Pathway residue is the main
...
the gradience but does not predict the trajectory of entry, the
...
The category's richness comes from the intersection of three overlapping
nodes. \term{interjection\textsubscript{syn}} supplies distributional
projectibility, \term{interjection\textsubscript{sem}} supplies
use-conditional and expressive projectibility, and
\term{interjection\textsubscript{int}} supplies sequential and interactional
projectibility. Interjections are thin as a syntactic category and rich
...
English interjections are a thin but projectible causal-pragmatic kind:
a stable projectible cluster recruited by diachronic pathways
(grammaticalization, pragmaticalization, nonce coinage) and structured
by synchronic links (prosodic-syntactic coupling, the
non-referentiality ratchet, interactional routine entrenchment, and
possibly regional/indexical conditioning).

What does categorizing something as an interjection let speakers,
hearers, and analysts predict? The central answer is pragmatic:
prosodic behaviour, interactional-pragmatic distribution, and stance
expression, rather than truth-conditional denotation. The
field-relative decomposition (\term{interjection\textsubscript{syn}},
\term{interjection\textsubscript{sem}}, \term{interjection\textsubscript{int}})
shows that the same lexeme can supply tokens that occupy distinct nodes
for different fields.

The category that Jespersen dismissed and Huddleston \& Pullum set aside
therefore matters because it separates several levels of evidence. The
projection target is clear: token-level inference. The stable profile is
the familiar cluster of prosodic, syntactic, semantic, and pragmatic
diagnostics. The network order comes from links among those diagnostics.
Routine entrenchment and regional/indexical conditioning are maintenance
hypotheses. Corrective control remains unshown.

For historical pragmatics specifically, the payoff is path dependence.
Interjection formation involves at least three diachronic processes:
...
onomatopoeia. These routes converge on a recognizable category position
while leaving visible residues. \mention{Gee}, \mention{damn these
mosquitoes!}, \mention{bruh}, \mention{wowed}, and \mention{fie} matter
because they show projectible profiles, boundary behaviour, exit
residues, and failure conditions.

The Boydian lesson survives the calibration. Fuzzy categories can be
inductively useful when their properties are connected
non-accidentally. Interjections also show why \term{homeostasis} should
not be used as a synonym for durable clustering. The present evidence
supports projection targets, stable profiles, network order, and
maintenance hypotheses. A stricter homeostatic claim would require an
explicit perturbation-and-correction story that the present paper
doesn't yet provide.
...
paper reanalyses that material through a causal-network lens.

Supplementary materials (data and analysis code for the corpus tests)
are available at
\url{https://github.com/BrettRey/interjections-hpc}.

I drafted this paper with the assistance of Claude (Anthropic). I've
...
\begin{itemize}
  \item \texttt{m0} (unconditional): \texttt{count \textasciitilde{} 1 + offset(log(words\_m)) + (1~|~item)}
  \item \texttt{m1} (conditioned): \texttt{count \textasciitilde{} 1 + offset(log(words\_m)) + (1~|~item) + (1~|~country)}
  \item \texttt{m2} (item type): \texttt{count \textasciitilde{} 1 + item\_type + offset(log(words\_m)) + (1~|~item) + (1~|~country)}
  \item \texttt{m3} (exploratory, country-by-type slopes): \texttt{count \textasciitilde{} 1 + item\_type + offset(log(words\_m)) + (1~|~item) + (1~+~item\_type~||~country)}
\end{itemize}
...
(m2) does not improve on m1 because the item random effect already
captures between-item variation. The exploratory model (m3) is excluded from
...
random-effect SD is positive across the interval. The SE is large relative to the
...
the primary evidence for P5; the model suggests the pattern survives
...
parsed KWIC exports. Chart requests for \mention{oohed} and
\mention{oohing} returned server errors, but KWIC exports loaded and
were used for coding. Minor chart/KWIC row-count discrepancies are
...
secondary codes were not used in the confirmatory test.
...
diachronic model was not estimated because semantic residue had no
...
\term{interjection\textsubscript{syn}} (syntactic),
\term{interjection\textsubscript{sem}} (semantic/use-conditional), and
\term{interjection\textsubscript{int}} (interactional-pragmatic). Coding
criteria are in the pre-registration. The decade column was hidden
during coding.
...
the ED50 ($\tau$: the year at which each diagnostic's probability
crosses 0.5):
