...
uncertainty than a bare figure suggests, and it is worth saying which
...
band is not a result at all. No two of those items have separable intervals,
so \mention{uh} at 0.76 is not distinguishable from \mention{yeah} at 0.55,
...
near zero and the coefficient becomes unstable, which is why
\mention{lah}'s interval runs from 0.89 to 4.27. Their concentration is
...
both concentrate where they are homographs of substrate words:
...
compares. The regional items carry the most exposure, since they are
...
from twenty-token samples and are too coarse to correct with, but they are
...
The counts are not all of one kind either, and the reason is worth stating
...
items are restricted to the interjection tag, but restriction is not the same as
...
Counts for \mention{haba} and \mention{yaar} cannot be tag-restricted at all,
because
...
that includes the proper noun. The regional figures are therefore raw strings
where the core figures are tag-restricted, which inflates the regional items
relative to the items they are set beside, though by less than it would if the
...
third, duplication, is a known property of the corpus. This is a limit imposed by the tagger rather than a choice: a
