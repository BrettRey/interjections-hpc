...
\citep{reynolds2026notEveryStableCluster}\fi: the five questions set out in
Section~\ref{sec:intro}. They're easier to see running on a single form.
\mention{Oof} is a recent arrival in standalone use. The projection target
is what calling it an interjection buys us: that it stands prosodically
separate, doesn't inflect, doesn't take a determiner, and carries stance
rather than reference. The stable profile is the observation that those
properties do travel together, in \mention{oof} as in \mention{ouch} and
\mention{wow}. Network order is the claim that they travel together for a
reason rather than by coincidence. A stabilizer would be whatever keeps them
travelling together while individual forms come and go. And corrective
control would be a process that pulled \mention{oof} back into the profile
if something pushed it out. That last rung is what \term{homeostasis} asks
for. I don't claim it: projectibility is the payoff and it arrives at the
third rung, while nothing below identifies a corrective process.
