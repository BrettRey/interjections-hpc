...
**Resolved 2026-08-11: repointed to Plessow.** `bullokar1586` in the central bib now cites Plessow 1906 (Palaestra 52, Berlin: Mayer & Müller) with `origdate = 1586`, so p. 373 is the page it actually names. Applied with `push_bib.py --update bullokar1586`, which is the sanctioned route: it writes with ordinary file I/O, so the hook protecting the central bib was neither removed nor weakened. Central commit `129fa26`.
