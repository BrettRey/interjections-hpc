...
in its standalone use rather than across the whole word family.

The warrant is whatever supports that expectation, and here it is thin: analogy
to \mention{ouch} and \mention{wow}, and the links set out below.

The profile is the relation the declaration posits, and its stability is the
separate observation that the same combination recurs across forms and over
time. Network order is a further and independent claim, that the properties are
linked rather than merely co-occurring.

Grounds explain how the form got there and why it stays. \textcite{oed_oof}
derives \mention{oof} as an imitative formation, which puts its entry on the
onomatopoeic route of Section~\ref{sec:recruitment} rather than on either
bleaching path. A stabilizer would be whatever keeps the combination together as
forms enter and leave, and corrective control would be a process that restored
it if something pushed \mention{oof} out.

That last is what \term{homeostasis} asks for. I don't claim it. Projectibility
needs the declaration warranted and the profile stable; it doesn't need a
corrective process, and nothing below identifies one.
