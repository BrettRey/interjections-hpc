...
semantics--pragmatics boundaries. His reason for caring anticipates mine.
Marginality is what makes the category informative, since \enquote{it is the
exception which proves the rule} \citep[57]{cram2008}. How a model handles
borderline cases reveals more than a run through the paradigm cases it was
designed for. The difference is in what the case is asked to reveal.
Cram asks what interjections show about our theories. I ask what they show
about categories. Interjections suit that question because no definitional
story survives for them. For noun or verb, a definition is always available as
a rival explanation of why the properties cluster, which hides whatever else is
holding them together. For interjections there's no such cover, so the
mechanism has to show itself.
...
That result isn't only about interjections. Where a definition is available it
explains the clustering, and whatever maintains the category stays out of
sight. Interjections have no such cover, so the maintaining links have to
appear, and here they turn out to be recruitment, coupling, and turnover. The
category grammars were least willing to admit is the one where you can watch a
category being held together.
