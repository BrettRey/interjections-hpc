...
What's graded is internal to the category. Members differ in how many cluster
properties they carry and in which field-relative nodes they satisfy, so the
predictions membership licenses vary in strength. What's fluid is the edge
across time, and \textcite[144]{boyd1999} builds that into the account.
\enquote{The properties that determine the conditions for falling under} a kind
term \enquote{may vary over time (or space)} while the term keeps its
...
than about precision. So the category is graded internally, sharp at the edge
for a fixed inquiry at a fixed time, and fluid across times as recruitment
moves forms in and out. Section~\ref{sec:prototype} turns on the difference
between that and fuzziness.
