...
\item \term{Grounds} explain why the profile obtains or
  ...
  license calling the cluster \term{homeostatic} (Boyd's term, discussed in
  Section~\ref{sec:causal-framework}), and the evidence here doesn't establish
  it.
\end{itemize}
...
stance force. The three nodes regroup those five by what each field needs the
category to predict, so the subscripts stay out of the way until
Section~\ref{sec:fieldrelative-nodes} defines them. Section~\ref{sec:projectibility} asks
...
loses prosodic-syntactic independence because it's embedded inside a
...
\textcite{khalidi2013,khalidi2018nodes} argues that natural kinds are
nodes in causal structures. A kind can be unified by causal relations rather
than by an essence, and those relations may be local mutual-reinforcement
relations, one-directional pathways, or converging historical routes. Each of
...
\textcite{oed_oof} attests it from 1777 and derives it as an imitative
formation, which puts it on the onomatopoeic route of
Section~\ref{sec:recruitment} rather than on either bleaching path. Its
prominence is more recent: COHA has three tokens before 1900, then a rise from
0.24 per million in the 1970s to 1.35 in the 2010s.\footnote{Raw string
  frequencies, not disambiguated by sense; the \textit{OED} records occasional
  nominal use.}

The declaration is what calling it an interjection commits us to: having
...
contribute use-conditionally rather than descriptively, in present-day English,
in its standalone use rather than across the whole word family. The warrant is whatever supports that expectation, and here it is
thin: analogy to \mention{ouch} and \mention{wow}, and the links set out
below. The profile is the relation the declaration posits, and
its stability is the separate observation that the same combination recurs
across forms and over time. Network order is a further and independent claim,
that the properties are linked rather than merely co-occurring. Grounds
explain the persistence: a stabilizer would be whatever keeps the combination
together as forms enter and leave, and corrective control would be a process
that restored it if something pushed \mention{oof} out. That last is what
\term{homeostasis} asks for. I don't claim it. Projectibility needs the
declaration warranted and the profile stable; it doesn't need a corrective
process, and nothing below identifies one.

The framework also clarifies how the argument relates to
\term{homeostatic-property-cluster theory}. \textcite{boyd1999} showed
that categories with no definitional necessary and sufficient conditions can
be useful and projectible when their properties are connected
non-accidentally. Interjections have that shape:
...
a categorial change, not that prosody enabled the syntax. She also limits it: the step from
\mention{Jesus!} to \mention{gee!} \enquote{can therefore not be explained
with reference to Invited Inferencing or syntactic position}
\citep[88]{gehweiler2008}.
...
\textcite{potts2007}'s expressives. \term{interjection\textsubscript{int}}
...
is the kind of prediction each supports. The syntactic node predicts
distribution: that a form will sit outside the clause and resist embedding.
The semantic node predicts meaning type: that its
contribution will be use-conditional rather than descriptive. The interactional node predicts sequence: where in a turn the form
falls, and what it invites next.
