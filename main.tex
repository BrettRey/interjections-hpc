...
One form makes the four concrete. Take \mention{gee}, as in
(\ref{ex:gee}).

\ea\label{ex:gee} \exmention{\exfocus{Gee}, that's a lot of money.}
\z

The declaration is the claim itself. Observing that \mention{gee} is set off
from what follows and never inflects, with no *\mention{geed} and no
*\mention{gees}, predicts three things not yet observed: that it will work as a
supplement, that its contribution will be use-conditional rather than
descriptive, and that it will turn up at the front of a turn.

The warrant is a separate question, about what licenses that inference and how
strongly. Table~\ref{tab:link-status} answers it link by link. It also says what
would defeat the inference: \mention{gee} taking a complement, on the model of
*\mention{gee that it's expensive}, or acquiring inflection.

The profile is the combination itself, and it isn't peculiar to \mention{gee}.
\mention{Wow}, \mention{ouch}, and \mention{hey} show the same package, and the
package has recurred over time. Network order is a further and stronger claim,
that these properties aren't merely found together but linked, so that being
prosodically set off makes syntactic non-integration more available.

The ground is whatever put \mention{gee} in the cluster, which in its case was
bleaching from a religious invocation (Section~\ref{sec:recruitment}). Whether
anything now holds it there is separate again, and nothing below shows the
combination being restored after disturbance.
...
category looks like when no definition is holding it together. What is peculiar
is where the predictions land. The label
\enquote{interjection} supports predictions about use, and those
...
interventions are connected tightly enough to support induction. Adopting that
structure settles nothing about boundaries, and
\textcite[66]{khalidi2013} is explicit that cluster kinds \enquote{need not be
structured in such a way as to allow intermediate cases}, though he holds that
some natural kinds do have fuzzy boundaries without forfeiting their
naturalness. So the network account and the sharp edge assumed in
Section~\ref{sec:cooccurrence} are compatible, and nothing below
turns on which way that question goes.
The
clustering matters because it lets us predict properties of new
instances from observed properties of old ones.

The claim is methodological. Nothing here turns on whether interjection is a
kind in a metaphysician's sense, and I don't argue that it is one. What the
category has to be is projectible for linguistic inquiry: a position in the
network whose causal links explain why particular observations support
particular predictions.

The four objects distinguished in Section~\ref{sec:intro} are easier to see
running on a single form. \mention{Oof} is a recent arrival in standalone use.
...
family. The warrant is whatever supports that expectation, which for a form
this recent is thin: analogy to \mention{ouch} and \mention{wow}, and the
