...
10. **R1 punch list, partly done 2026-07-24.** Applied: *goddammit* relabelled as fossilized syntax rather than morphology with a cross-reference; introducing sentences added before both previously-unannounced tables; R1's anti-contraction note still rejected. **Left for Brett, each needing analytical judgement rather than a mechanical edit:** the *hello* interjection-vs-routine-formula inconsistency (possibly defensible, since the paper follows Wilkins in treating routine formulae as an interjection subtype); the token/type point about *oh* (where "change-of-state token" is Heritage's established term and R1 may have misread it); the agreement-direction wording (no matching string in the current text); and R1's substantive challenge about whether a hearer must identify a form as an interjection for the predictions to run (*Bummer!*, *groovy*), which needs a form-driven vs category-driven distinction and is closer to a new subsection than a fix. See `notes/2026-07-23-jop-rejection-citation-integrity.md`.
