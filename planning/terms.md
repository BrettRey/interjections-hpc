...
| causal-pragmatic (as a modifier) | causal network / causal structure / causally ordered | Retired 2026-07-25 in a second pass. Three reasons: it drifted against Khalidi's *causal network*, which the paper uses 28 times, to the point where one sentence wrote `\ref{fig:causal-network}` while calling the figure "the proposed causal-pragmatic network"; it named the whole network by one of its domains when only one of the four synchronic links (routine entrenchment) is squarely pragmatic, which contradicts the field-relative decomposition; and it was the last residue of the pragmatics frame, the same phrase as the retired subtitle "Stable Causal-Pragmatic Clustering". Zero uses remain. |
