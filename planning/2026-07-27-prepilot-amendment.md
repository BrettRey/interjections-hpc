...
### 3. Audio is a real measurement gate
Every timing field records its source and resolution. CABNC is an existing recorded corpus, ethically on a par with a written corpus, so no REB review is required. Before sampling, Gate 0 records the AudioBNC research-use terms and BNC User Licence, confirms authenticated access and licence-compliant local storage, and requires a 40-event audit spanning at least eight pilot blocks.
...
The protocol, schemas, validators, tests, 180-ms span parser, census rerun, and local acoustic-preparation utility are implemented. The next empirical step is Gate 0: record the corpus licence, confirm authenticated audio access and licence-compliant storage, and run the 40-event waveform/alignment audit. Retrieval, variant, component, pathway, and sign-type dossier tables can then be frozen in the declared order. No annotation-packet generation, post-offset outcome coding, or manuscript reconstruction begins before the applicable audio, model-panel, agreement/audit, and leakage gates are satisfied.
