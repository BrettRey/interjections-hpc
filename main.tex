...
Malaysian and Singaporean \mention{lah} carries the semantic and interactional
nodes without the syntactic one, for the reason given in
...
They are not, however, the same kind of case, and the difference matters for
...
\citep[69]{unuabonahdaniel2020}. \mention{Lah} is not. It is clause-final and
...
\mention{lah} is one of the few particles \textcite[463]{lim2007} does not
...
therefore recruit a form to the interactional node without recruiting it to the
syntactic one, which is what \mention{lah} looks like and \mention{haba} does
not. The GloWbE evidence in
...
\footnote{Part-of-speech--tagged frequencies in COHA \citep{davies2010},
  ...
  tagged list for that decade, 111 forms, so it does not depend on the
...
\begin{figure}[htbp]
  ...
  \caption{The inventory turns over while the category position holds. Each
    ...
    forms at zero remain visible. Extracted 2026-07-27 and archived under
...
uncertainty than a bare figure suggests, and it's worth saying which
comparisons survive it. Bootstrapping over the twenty sections, the
...
relative to the items they're set beside, though by less than it would if the
tag were reliable. The size of that inflation is indicated by
...
They reach the smaller figure by inspecting each hit: their data were
\enquote{manually searched in order to eliminate cases in which the BIs were
found in utterances made in Nigerian Pidgin}, in an indigenous Nigerian
language, or in posts repeated by copying \citep[69]{unuabonahdaniel2020}.
...
Since the hand audit of \mention{ha} in Section~\ref{sec:discussion} shows
that one item's counts carry a validity rate that varies by country, the model
was refitted with that error propagated rather than assumed away. Each
country's rate enters as a draw from its \textsc{beta} posterior, thinning
\mention{ha}'s count while leaving the other seventeen items untouched, and
the pair of models is refitted over draws. Correcting it moves little: the
country random-effect SD goes from 0.42 to 0.44 with iterated laughter counted
and 0.45 with it excluded, and the model comparison moves from 28.8 to 26.9
and 28.5 across the same specifications. Two limits on that reassurance. The
refits use approximate rather than exact leave-one-out, so their absolute
values are not comparable with the figures above and only the internal
contrast is informative. And a hand-coded concordance measures precision, so
this corrects tokens the tagger wrongly admitted and not those it missed.
